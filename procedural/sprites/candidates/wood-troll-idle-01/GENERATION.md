...
## DR consumer integration checkpoint

DR commit 7e645d6f integrates the six selected frames through the existing exact-name scene sprite registry. Mounted 1440px client playback observed all six frames and maintained the tactical anchor within one pixel. Initial widthScale .95 was too short; selected 1.4 renders about 114px against the 91px player, matching a noticeably taller troll. The screenshot was inspected in actual gameplay UI. Source review metadata remains candidate for general shared-catalog admission; this narrower DR consumer admission is separate. The loop is cosmetic breathing, head tilt and blink only. All six decoded frames have transparent pixels, opaque subject pixels, and zero visible keylike-magenta pixels under the extractor's threshold. Production build and focused art tests passed.
