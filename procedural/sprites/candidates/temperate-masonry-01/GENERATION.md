...
## Connector assembly followup

`kit.json` now records named estimated edge endpoints and footprint polygons for the straight wall, corner and both doorways. `build-assembly-preview.mjs` imports the canonical shared `alignSpriteEdge` solver and generates the self-contained `assembly-preview.html`. Regenerate with `node procedural/sprites/candidates/temperate-masonry-01/build-assembly-preview.mjs` from repository root. No duplicate alignment solver.

Source-only browser capture `assembly-preview.png` inspected: wall–arch–wall forms a readable base-abutment arrangement, with zero endpoint error. This does NOT prove a seamless kit: left and right wall scales differ (0.4236 versus 0.4519), visible stone-course scale differs from the tall arch, and the base direction changes by 6.3 degrees. Door jambs cover the junction but artwork should be normalized in a later production revision. Timber frontage is explicitly refused for straight continuation due to 34.6-degree opposite-slope mismatch. No rotation or mirroring used. Cyan footprints are estimates, not approved collision; yellow endpoint overlays make the mathematical join inspectable. Initial capture timed out because the helper waits for a main element; added semantic main and the subsequent source capture passed, browser closed. No gameplay suite run.
