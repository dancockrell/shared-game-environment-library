...
try {
    while (-not $process.HasExited) {
        ...
        $samples.Add([pscustomobject][ordered]@{seconds=$watch.Elapsed.TotalSeconds; processTreeWorkingSetMiB=$treeWorkingSet/1MB; wholeGpuUsedMiB=(Get-GpuSample)})
...
$receipt = [ordered]@{
    schemaVersion=1; purpose='Unmodified upstream hanging-cloth feasibility benchmark, not garment or art admission'
    ...
    exitCode=$exitCode; timedOut=$timedOut; elapsedSeconds=$watch.Elapsed.TotalSeconds
