...
<script>const m=${data};document.querySelector('#title').textContent=m.title;const stage=document.querySelector('#stage'),img=new Image();let frame=0,playing=false,last=0,ready=false;const speed=document.querySelector('#fps');speed.value=String(m.fps);
...
img.onload=()=>{ready=true;for(let i=0;i<m.frames.length;i++){const c=document.createElement('canvas');c.width=240;c.height=220;c.setAttribute('aria-label','Frame '+(i+1));document.querySelector('#film').append(c);draw(c,i)}paint();document.body.dataset.ready='true'};
img.onerror=()=>document.querySelector('#error').textContent='Source image could not load';img.src=m.image.path;
