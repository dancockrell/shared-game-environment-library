...
func build() -> void:
	...
	if args.size() == 2 and args[1] == "--assemble-catalog":
		...
		return
	if args.size() == 4 and args[1] == "--assemble-catalog":
		assert(args[2].is_absolute_path() and args[3].is_absolute_path())
		var recipe: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(args[2]))
		recipe["sourceRecipeSha256"] = FileAccess.get_sha256(args[2])
		await assemble_catalog(recipe,args[3])
		return
	if args.size() == 2 and args[1] == "--inspect-catalog":
		inspect_catalog()
		return
	if args.size() == 3 and args[1] == "--inspect-catalog":
		assert(args[2].is_absolute_path())
		inspect_catalog(args[2])
		return
...
func build_catalog() -> void:
	...
	if specs.size() > 60:
		assert(sheet.get_region(Rect2i(0,8400,2560,sheet_height-8400)).save_png(folder.path_join("landscape-and-trade.png")) == OK)
		assert(rear_sheet.get_region(Rect2i(0,8400,2560,sheet_height-8400)).save_png(folder.path_join("landscape-and-trade-rear.png")) == OK)
...
func assemble_catalog(recipe: Dictionary = {}, destination: String = "") -> void:
	var folder := output_dir.path_join("catalog")
	var result_dir := folder if destination.is_empty() else destination
	assert(DirAccess.make_dir_recursive_absolute(result_dir) == OK)
	...
	assembly.name = recipe.get("sceneName","WorkshopQuay_AssemblyTest_NotMudMap")
	kit.root.add_child(assembly)
	for x in range(-4,5):
		for z in range(-2,4):
			saved_instance(source,records,"cobble-plaza",Vector3(x*4,0,z*4),assembly)
	var buildings: Array[Node3D] = []
	for item in recipe.get("buildings",[["smithy",-10.0,4.0],["warehouse",0.0,4.0],["bell-hall",10.0,4.0]]):
		buildings.append(saved_instance(source,records,item[0],Vector3(item[1],0.22,item[2]),assembly))
	...
	var capstan := saved_instance(source,records,"capstan",low_pier.position+Vector3(0,1.145,-0.65),assembly)
	assert(absf(capstan.position.y-deck_y) < 0.001)
	...
	for frame in 32:
		await process_frame
	RenderingServer.force_draw(false)
	assert(root.get_texture().get_image().save_png(result_dir.path_join("assembly-workshop-quay.png")) == OK)
	...
	var packed := PackedScene.new()
	assert(packed.pack(kit.root) == OK)
	assert(ResourceSaver.save(packed,result_dir.path_join("assembly-workshop-quay.scn")) == OK)
	var check := (load(result_dir.path_join("assembly-workshop-quay.scn")) as PackedScene).instantiate()
	assert(collect_meshes(check).size() == collect_meshes(kit.root).size())
	check.free()
	var evidence := FileAccess.open(result_dir.path_join("assembly-check.json"),FileAccess.WRITE)
	evidence.store_string(JSON.stringify({"sceneName":str(assembly.name),"scope":recipe.get("scope","Reusable asset composition and socket test only; not The Crossing topology"),"sourceRecipeSha256":recipe.get("sourceRecipeSha256",null),"serviceCreditsConsumed":0,"sourceCatalogSha256":FileAccess.get_sha256(folder.path_join(report.native)),"placements":placements,"socketJoinErrorsMeters":join_errors,"buildingEnvelopeOverlap":false,"sourceGeometryRegenerated":false,"nativeReloadMeshCount":collect_meshes(kit.root).size()},"\t")+"\n")
	evidence.close()
	source.free()
...
func inspect_catalog(assembly_directory: String = "") -> void:
	...
	# A consumer verifies its saved catalog, even while new recipes are authored.
	if assembly_directory.is_empty():
	...
	loaded.free()
	var catalog_path := folder.path_join(report.native)
	if not assembly_directory.is_empty():
		folder = assembly_directory
	if FileAccess.file_exists(folder.path_join("assembly-check.json")):
		...
		assert(evidence.sourceCatalogSha256 == FileAccess.get_sha256(catalog_path),"Assembly must be rebuilt after catalog changes")
		...
		var placed := saved.get_node(NodePath(evidence.get("sceneName","WorkshopQuay_AssemblyTest_NotMudMap")))
