...
Prepared GLBs contain hidden alternative bodies/outfits. Apply the matching profile; a raw viewer may show overlapping alternatives. Actor export strips unused visual resources while retaining transform/attachment structure and the authoring recipe. Rebuild outfit changes from the original source; exported hidden alternatives no longer carry geometry. Explicitly classified construction morphs are baked only when every shape on that mesh is classified; unclassified animation/expression shapes are preserved. The full rig remains, so this is not a crowd-optimized LOD. Inspect actual triangle, texture and retained-resource budgets.
