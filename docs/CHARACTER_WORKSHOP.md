...
| Sewing construction baseline | Original metric panel triangulation/refinement, explicit edge stitches, XPBD distance-constraint relaxation, per-panel mesh surfaces and metric UVs | Verified sewing coupon only; no gravity/time integration, bending, body/self-contact, gathers, garment skinning or finished costume |
...
The runner now executes the sewing baseline before character builds, records its tool hashes and retains a saved mesh and JSON receipt. The coupon measures maximum seam gap and relative edge-length error; these are not garment-fit measurements. Still missing: quantified garment/body penetration, self-intersections, shear error, broad motion clearance, render similarity and measured VRAM. These are unmeasured, not zero. Do not label repeated importer tests as proof that tailoring works.
...
The quasistatic distance solve uses XPBD accumulated multipliers, inverse masses, compliance and a timestep parameter. Multipliers persist across iterations of one solve. This is **not** a full small-step cloth simulator: there is no velocity integration, gravity, bending or collision response. The original cut/placement is not mutated. Invalid masses, positions and constraint indices reject before solving.
